...
def searching (message) :
    ...
    if len(links) == 0 :
        ...
        item = telebot.types.InlineKeyboardMarkup().add(telebot.types.InlineKeyboardButton('ссылка',url=req))
        bot.send_message(message.from_user.id,'Ссылка на поисковый запрос: ',reply_markup=item)
        ...
        if address['city_district'] != None : #чтоб если адрес неправильный, он не сломался
            ...
            if isinstance(address['metro'],list) : #может быть несколько станций метро или одна
                for i in address['metro'] :
                    prms['metro[]'].append(Metro.get(i['name'].strip().lower(),'kurskaya')) #чтоб если чего-то нет в списке, он не сломался
            else :
                prms['metro[]'] = Metro.get(address['metro'],'kurskaya')
            prms['district[]'] = Districts.get(district,'basmannyy') #чтоб если чего-то нет в списке, он не сломался
            print(prms)
            bot.send_message(message.from_user.id,emoji.emojize(':ok_hand: \n\n',use_aliases=True),reply_markup=menu())
        else :
